...
from models.resnet_cifar10 import resnet_56, resnet_110, resnet_56_spa, resnet_110_spa
from models.vgg_cifar10 import vgg_16_bn_spa, vgg_16_bn

from data import cifar10
import utils
from prepare_prune import prepare_prune

parser = argparse.ArgumentParser("CIFAR-10 training")
parser.add_argument('--data_dir', type=str, default='./data', help='path to dataset')
parser.add_argument('--arch', type=str, default='resnet_56_spa',
                    choices=('vgg_16_bn', 'resnet_56', 'resnet_110', 'resnet_50', 'resnet_56_spa'),
...
parser.add_argument('--threshold', type=float, default=0.5, help='information retain threshold')
args = parser.parse_args()
CLASSES = 10

args.data_dir = '../../data'
args.arch = 'vgg_16_bn_spa'
args.lr_type = 'cos'
args.result_dir = 'train_from_raw_vgg_16_bn_cifar_with_ccm_rl_0.01_lambda_0.01_the_best/pruned_result/CCM_finetune_VGG_16_threshold_0.6'
args.batch_size = 256
args.epochs = 600
args.label_smooth = 0
...
args.pretrain_dir = 'train_from_raw_vgg_16_bn_cifar_with_ccm_rl_0.01_lambda_0.01_the_best/checkpoint.pth.tar'
args.ci_dir = 'train_from_raw_vgg_16_bn_cifar_with_ccm_rl_0.01_lambda_0.01_the_best/CI_vgg_16_bn'
args.gpu = '0'
args.threshold = 0.6
...
def load_resnet_model(model, oristate_dict, layer, threshold):
    cfg = {
        56: [9, 9, 9],
        110: [18, 18, 18],
    }
...
def main():
    ...
    # calculate sparsity
    channel_reserved_by_CI, sparsity = prepare_prune(args.ci_dir, 12, args.threshold)
    ...
    if args.arch == 'vgg_16_bn_spa':
        model = eval(args.arch)(sparsity=sparsity, sparsity_type='ccm').cuda()
    logger.info(model)
    print(model)
    ...
    target_model_flops, target_model_params = profile(model, inputs=(input_image,))
    logger.info('Params: %.2f' % (target_model_params))
    logger.info('Flops: %.2f' % (target_model_flops))
    ...
    origin_model = vgg_16_bn_spa(sparsity=[0.] * 100, sparsity_type='original').cuda()
    # calculate model size
    input_image_size = 32
    input_image = torch.randn(1, 3, input_image_size, input_image_size).cuda()
    origin_model_flops, origin_model_params = profile(origin_model, inputs=(input_image,))
    logger.info('Params: %.2f' % (origin_model_params))
    logger.info('Flops: %.2f' % (origin_model_flops))
    print(origin_model)
    ...
    elif args.arch == 'resnet_56_spa':
        load_resnet_model(model, oristate_dict, 56, args.threshold)
    elif args.arch == 'resnet_110_spa':
        load_resnet_model(model, oristate_dict, 110, args.threshold)
    else:
        raise

    learning_rate = args.learning_rate * (target_model_params/origin_model_params)
